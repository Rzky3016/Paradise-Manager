...
def ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member):
        message.reply_text("Saya sangat berharap saya bisa membunuh admin...")
        return ""
    
    if user_id == 1118936839:
        message.reply_text("Tidak mungkin saya bisa membunuh pengguna ini. Dia adalah Pencipta/Pengembang saya")
        return ""
    
    if user_id == bot.id:
        message.reply_text("Aku tidak akan melarang diriku sendiri..BODOH!")
        return ""
    ...
    try:
        ...
        bot.sendMessage(chat.id, "👨‍💻 {} berhasil dibunuh oleh {}!".format(mention_html(member.user.id, member.user.first_name), mention_html(user.id, user.first_name)),
        ...
        if excp.message == "Pesan balasan tidak ditemukan":
            ...
            LOGGER.exception("ERROR banning user %s in chat %s (%s) due to %s", user_id, chat.title, chat.id,
                             excp.message)
            message.reply_text("Saya tidak bisa membunuh pengguna itu.")
...
def temp_ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id == bot.id:
        message.reply_text("Aku tidak akan melarang diriku sendiri, apakah lo gila?")
        return ""

    if not reason:
        message.reply_text("Anda belum menentukan waktu untuk membunuh pengguna ini!")
        return ""
    ...
    try:
        ...
        if excp.message == "Pesan balasan tidak ditemukan":
            # Do not reply
            message.reply_text("Berhasil di BAN! Pengguna akan diblokir {}.".format(time_val), quote=False)
            return log
        else:
            LOGGER.warning(update)
            LOGGER.exception("ERROR banning user %s in chat %s (%s) due to %s", user_id, chat.title, chat.id,
                             excp.message)
            message.reply_text("Bodoh, saya tidak bisa melarang pengguna itu.")
...
def kick(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if is_user_ban_protected(chat, user_id):
        message.reply_text("Saya sangat berharap saya bisa menendang pantat admin...")
        return ""

    if user_id == bot.id:
        message.reply_text("Bodoh, aku tidak akan melakukan itu")
        return ""
    ...
    if res:
        ...
        log = "<b>{}:</b>" \
              ...
              "\n<b>User:</b> {} (<code>{}</code>)".format(html.escape(chat.title),
                                                           ...
                                                           member.user.id)
        if reason:
            log += "\n<b>Karena:</b> {}".format(reason)

        return log

    else:
        message.reply_text("Bodoh, saya tidak bisa menendang pengguna itu.")
...
def kickme(bot: Bot, update: Update):
    ...
    if res:
        update.effective_message.reply_text("Menendang pantat pengguna dari group ini.")
    else:
...
def banme(bot: Bot, update: Update):
    ...
    if res:
        update.effective_message.reply_text("Tidak masalah, penggunan di BAN.")
        log = "<b>{}:</b>" \
              "\n#BANME" \
              "\n<b>User:</b> {}" \
...
def unban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if is_user_in_chat(chat, user_id):
        message.reply_text("Mengapa Anda mencoba membatalkan pelarangan seseorang yang sudah ada di obrolan?")
        return ""

    chat.unban_member(user_id)
    message.reply_text("Oke, pengguna bisa masuk kembali!")
...
def selfunban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if is_user_in_chat(chat, user.id):
        message.reply_text("Bukankah kamu sudah obrolan chat??")
        return

    chat.unban_member(user.id)
    message.reply_text("Ya, saya telah membatalkan pelarangan Anda.")
